<template>
  <composite-form-dialog
    ...
    <a-form-model-item v-if="mode != 'editUser'" :label="$t('Discount.Discount')" prop="discount">
      <a-input v-model="discountForm.discount" />
    </a-form-model-item>
    <a-form-model-item v-if="mode != 'editDiscount'" :label="$t('Discount.Type.User')" prop="names">
      <multi-tags-input
...
export default {
  ...
  data() {
    return {
      ...
      nameRule: [
        ValidRoleFactory.getLengthRoleForText(this.$t('Discount.Type.User'), 3, 32),
        ValidRoleFactory.getValidUserameForText(this.$t('Discount.Type.User'), true),
      ],
      discountRules: {
        names: [
          ValidRoleFactory.getRequireRoleForArray(this.$t('Discount.Type.User')),
          ValidRoleFactory.getLengthRoleForArray(this.$t('Discount.Type.User'), 0, 200),
          ValidRoleFactory.getUniqueRoleForArray(this.$t('Discount.Type.User')),
        ],
        discount: [
  ...
  methods: {
    ...
    successMessageFormatter(res) {
      ...
      if (this.mode === 'editUser') {
        return this.$t('Discount.Edit.User.Success')
      }
    },
    ...
    doCreate() {
      this.mode = 'create'
      this.title = this.$t('Discount.User.Create.Title')
      this.discountForm = {
        names: [],
        type: 'user',
        discount: '',
    ...
    doEditDiscount(users, discount) {
      ...
      this.title = this.$t('Discount.User.Edit.Title')
      this.discountForm = {
        names: users,
        type: 'user',
    ...
    doEditUser(users, discount) {
      this.editqueue = discount
      this.mode = 'editUser'
      this.title = this.$t('Discount.User.Edit')
      this.discountForm = {
        names: users,
        type: 'user',
        discount: discount[0].discount,
